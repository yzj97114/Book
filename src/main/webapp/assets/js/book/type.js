...
function goUpdate(id) {
    sys.ajax.doAjax({
        type: 'get',
        url: ctx + "book/type/detail?bookTypeId=" + id,
        async: false,//同步
        dataType: 'json',
...
function popBox(id) {
    var title = '新增图书类型';
    var requestUrl = ctx + "book/type/add";
    if (sys.util.isEmpty(id)) {//添加
        $(".popBox input[name='id']").val("");
    } else {//修改
        title = '修改图书类型';
        requestUrl = ctx + "book/type/update";
                ...
                success: function (result) {
                    if (result.code == 0) {
                        sysLayer.util.alert("保存成功", {
                            ok: function () {
                                layer.close(index);
                                window.location.href = ctx + "book/type/list";
                            }
                        });
                    } else {
                        sysLayer.util.alert(result.msg);
